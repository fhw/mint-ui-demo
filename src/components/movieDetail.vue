    ...
    <div class="movie-card">
      ...
      <section class="movie-intro">
        <div class="left">
          <p>
            主演：<br>
            <span v-for="(castsName, index) in detail.casts" :key="index">{{castsName.name}}/</span>
          </p>
          <p>
            导演：<br>
            <span v-for="(dName, index) in detail.directors" :key="index">{{dName.name}}/</span>
          </p>
          <mt-cell>
            <span v-if='detail.rating.average!=0'>{{detail.rating.average}}分</span>
            <span v-else>暂无评分</span>
            <img v-for="(starNum, index) in Math.round(detail.rating.average/2)" :key="index" slot="icon"
